...
public class AdminServiceImpl implements AdminService {
    ...
    public void saveAdmin(Admin admin) {
        ...
        // 更新か新規作成か判定
        if (admin.getId() != null) {
            existingAdmin = adminRepository.findById(admin.getId())
                    .orElseThrow(() -> new RuntimeException("Admin not found"));
        }

        // パスワードの処理（入力があった場合のみ更新）
        if (existingAdmin != null && admin.getPassword() != null && !admin.getPassword().isEmpty()) {
            admin.setPassword(passwordEncoder.encode(admin.getPassword()));
    ...
    public Admin getCurrentLoggedInAdmin() {
        ...
        if (principal instanceof UserDetails) {
            ...
            throw new RuntimeException("現在のログインユーザーが見つかりません");
        }
